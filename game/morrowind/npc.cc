...
static valuenames_t l_BloodTypes[] = {
	{
		MWESM_NPCFLAG_BLOODRED,
		_T("Default (Red)")
	},
	{
		MWESM_NPCFLAG_BLOODSKEL,
		_T("Skeleton (White)")
	},
	{
		MWESM_NPCFLAG_BLOODMETAL,
		_T("Metal (Gold)")
	},
	{
		MWESM_NPCFLAG_BLOODRED,
...
CEsmSubRecord *CEsmNpc::AllocNewSubRecord(const TCHAR *pType, const long RecordSize) {
	...
	if ( _stricmp(pType, MWESM_SUBREC_NPDT) != 0) {
...
int CEsmNpc::CompareFields(const int FieldID, CEsmRecord *pRecord) {
	...
	if (!pRecord->IsType(MWESM_REC_NPC_)) {
...
void CEsmNpc::CreateNPCData(const bool IsShort) {
	...
	if (IsShort && m_pNpcShortData != NULL) {
		return;
	}

	if (!IsShort && m_pNpcLongData != NULL) {
		return;
	}
	...
	if (m_pNpcLongData != NULL) {
		switch (AttrIndex) {
			case MWESM_ATTRIBUTE_STR:
				return (long)(byte)m_pNpcLongData->GetStrength();

			case MWESM_ATTRIBUTE_AGI:
				return (long)(byte)m_pNpcLongData->GetAgility();

			case MWESM_ATTRIBUTE_SPD:
				return (long)(byte)m_pNpcLongData->GetSpeed();

			case MWESM_ATTRIBUTE_END:
				return (long)(byte)m_pNpcLongData->GetEndurance();

			case MWESM_ATTRIBUTE_PER:
				return (long)(byte)m_pNpcLongData->GetPersonality();

			case MWESM_ATTRIBUTE_INT:
				return (long)(byte)m_pNpcLongData->GetIntelligence();

			case MWESM_ATTRIBUTE_WIL:
				return (long)(byte)m_pNpcLongData->GetWillpower();
...
void CEsmNpc::SetSkill(const TCHAR *pString) {
	...
	if (IsAutoCalc() || m_pNpcLongData == NULL) {
		return;
	}
...
void CEsmNpc::SetAutoCalc(const bool AutoCalcFlag) {
	...
	/* Set the actual flag */
	if (m_pFlag) {
		m_pFlag->SetFlag(MWESM_NPCFLAG_AUTOCALC, AutoCalcFlag);
	}

	/* Ignore if the flag hasn't changed */
	if (m_pNpcLongData != NULL && !AutoCalcFlag) {
		return;
	}

	if (m_pNpcShortData != NULL && AutoCalcFlag) {
		return;
	}
...
void CEsmNpc::SetHealth(const long Value) {
	if (m_pNpcLongData != NULL) {
		m_pNpcLongData->SetHealth((short)Value);
	}
}
...
void CEsmNpc::SetFatigue(const long Value) {
	if (m_pNpcLongData != NULL) {
		m_pNpcLongData->SetFatigue((short)Value);
	}
}
