...
class CEsmScriptError {
  ...
  public:
	...
	void SetToken(const TCHAR *pString) {
		m_Token = pString;
	}

	void SetTokenID(const int TokenID) {
		m_TokenID = TokenID;
	}

	void SetNumFuncArgs(const int Value) {
		m_NumFuncArgs = Value;
	}

	void SetFuncArgIndex(const int Index) {
		m_FuncArgIndex = Index;
	}
	...
	void SetCharacter(const int Char) {
		m_Character = Char;
	}
	...
	void SetTypeError(void) {
		m_Type = ESMSCR_MESSAGE_ERROR;
	}

	void SetTypeWarning(void) {
		m_Type = ESMSCR_MESSAGE_WARNING;
	}

	void SetFunction(esmscrfuncinfo_t *pFunction) {
