...
void CEsmFactionDlg::GetControlData() {
	...
	if (m_HiddenCheck.GetCheck()) {
		pFactionData->Flags = MWESM_FACTFLAG_HIDDEN;
	}
...
void CEsmFactionDlg::OnInitialUpdate() {
	...
	/* Rank list */
	m_RankList.InsertColumn(MWESM_RANKLIST_INDEX,
	                        _T("#"),
	                        LVCFMT_CENTER,
	                        40,
	                        MWESM_RANKLIST_INDEX);
	m_RankList.InsertColumn(MWESM_RANKLIST_NAME,
	                        _T("Rank Name"),
	                        LVCFMT_LEFT,
	                        100,
	                        MWESM_RANKLIST_NAME);
	m_RankList.InsertColumn(MWESM_RANKLIST_ATTRIB1,
	                        _T("Attrib 1"),
	                        LVCFMT_CENTER,
	                        60,
	                        MWESM_RANKLIST_ATTRIB1);
	m_RankList.InsertColumn(MWESM_RANKLIST_ATTRIB2,
	                        _T("Attrib 2"),
	                        LVCFMT_CENTER,
	                        60,
	                        MWESM_RANKLIST_ATTRIB2);
	m_RankList.InsertColumn(MWESM_RANKLIST_SKILL1,
	                        _T("Skill 1"),
	                        LVCFMT_CENTER,
	                        60,
	                        MWESM_RANKLIST_SKILL1);
	m_RankList.InsertColumn(MWESM_RANKLIST_SKILL2,
	                        _T("Skill 2"),
	                        LVCFMT_CENTER,
	                        60,
	                        MWESM_RANKLIST_SKILL2);
	m_RankList.InsertColumn(MWESM_RANKLIST_FACTREP,
	                        _T("Fact Rep"),
	                        LVCFMT_CENTER,
	                        60,
	                        MWESM_RANKLIST_FACTREP);
	m_RankList.SetExtendedStyle(LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES);

	/* Faction reaction list */
	m_ReactionList.InsertColumn(MWESM_REACLIST_NAME,
	                            _T("Faction"),
	                            LVCFMT_LEFT,
	                            140,
	                            MWESM_REACLIST_NAME);
	m_ReactionList.InsertColumn(MWESM_REACLIST_VALUE,
	                            _T("Reaction"),
	                            LVCFMT_CENTER,
	                            80,
	                            MWESM_REACLIST_VALUE);
...
int CEsmFactionDlg::OnUpdateItem(esmrecinfo_t *pRecInfo) {
	...
	if (pRecInfo->pRecord->IsType(MWESM_REC_FACT)) {
		...
		if (Index >= 0) {
			pRecInfo = (esmrecinfo_t *)m_FactionList.GetItemData(Index);
		}
