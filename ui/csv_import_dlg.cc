...
bool CEsmCsvImportDlg::CreateRecord(CCsvRow *pRow) {
	...
	for (ColIndex = 0; ColIndex < pHeaderRow->GetNumElements(); ColIndex++) {
		if (ColIndex == m_TypeColIndex) {
			continue;
		}

		if (ColIndex == m_IDColIndex) {
			continue;
		}
		...
		if (pString == NULL || pHeaderString == NULL) {
			continue;
		}
...
void CEsmCsvImportDlg::FillCsvList() {
	...
	for (ColIndex = 0; ColIndex < pRow->GetNumElements(); ColIndex++) {
		pString = pRow->GetAt(ColIndex);
		m_CsvList.InsertColumn(ColIndex + 1,
		                       pString == NULL ? _T("?") : static_cast<const char *>(*pString),
		                       LVCFMT_CENTER,
		                       ESMCSVIMP_LIST_COLWIDTH,
		                       ColIndex + 1);
	}
	...
	for (RowIndex = 1; RowIndex < m_CsvFile.GetNumLines(); RowIndex++) {
		...
		for (ColIndex = 0; ColIndex < pRow->GetNumElements(); ColIndex++) {
			...
			if (pString != NULL) {
				m_CsvList.SetItemText(ListIndex, ColIndex + 1, *pString);
			}
		}
	}
}
