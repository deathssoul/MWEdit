...
void FillEsmRecordCombo(CComboBox &ComboBox, const TCHAR *pType, const bool IncludeEmpty) {
	...
	if (pDocument == NULL || !pDocument->IsKindOf(RUNTIME_CLASS(CMWEditDoc))) {
		return;
	}
	...
	while (pRecInfo != NULL) {
		...
		if (ListIndex >= 0) {
			ComboBox.SetItemData(ListIndex, (DWORD)pRecInfo);
		}
...
void FillEsmGlobalTypeCombo(CComboBox &ComboBox) {
	...
	if (ListIndex >= 0) {
		ComboBox.SetItemData(ListIndex, MWESM_GLOBAL_SHORT);
	}

	ListIndex = ComboBox.AddString(_T("long"));

	if (ListIndex >= 0) {
		ComboBox.SetItemData(ListIndex, MWESM_GLOBAL_LONG);
	}

	ListIndex = ComboBox.AddString(_T("float"));

	if (ListIndex >= 0) {
		ComboBox.SetItemData(ListIndex, MWESM_GLOBAL_FLOAT);
	}
}
...
void FillEsmClassSpecCombo(CComboBox &ComboBox) {
	...
	if (ListIndex >= 0) {
		ComboBox.SetItemData(ListIndex, MWESM_CLASSSPEC_COMBAT);
	}

	ListIndex = ComboBox.AddString(GetESMClassSpec(MWESM_CLASSSPEC_MAGIC));

	if (ListIndex >= 0) {
		ComboBox.SetItemData(ListIndex, MWESM_CLASSSPEC_MAGIC);
	}

	ListIndex = ComboBox.AddString(GetESMClassSpec(MWESM_CLASSSPEC_STEALTH));

	if (ListIndex >= 0) {
		ComboBox.SetItemData(ListIndex, MWESM_CLASSSPEC_STEALTH);
	}
}
...
void FillEsmCreatureCombo(CComboBox &ComboBox, const bool IncludeEmpty, const bool IncludeLevel) {
	...
	if (IncludeEmpty) {
		...
		if (ListIndex >= 0) {
			ComboBox.SetItemData(ListIndex, NULL);
		}
	}
...
void FillEsmFunctionCombo(CComboBox &ComboBox) {
	...
	for (Index = 0; s_Functions[Index] != 0; Index++) {
		...
		if (ListIndex >= 0) {
			ComboBox.SetItemData(ListIndex, (DWORD)s_Functions[Index]);
		}
	}
}
...
void FillEsmGenderCombo(CComboBox &ComboBox, const bool IncludeEmpty) {
	...
	if (IncludeEmpty) {
		...
		if (ListIndex >= 0) {
			ComboBox.SetItemData(ListIndex, (DWORD)MWESM_INFOGENDER_NONE);
		}
	}
	...
	if (ListIndex >= 0) {
		ComboBox.SetItemData(ListIndex, (DWORD)MWESM_INFOGENDER_MALE);
	}

	ListIndex = ComboBox.AddString(GetESMInfoGender(MWESM_INFOGENDER_FEMALE));

	if (ListIndex >= 0) {
		ComboBox.SetItemData(ListIndex, (DWORD)MWESM_INFOGENDER_FEMALE);
	}
}
...
void FillEsmBloodTypeCombo(CComboBox &ComboBox) {
	...
	if (ListIndex >= 0) {
		ComboBox.SetItemData(ListIndex, MWESM_NPCFLAG_BLOODRED);
	}

	ListIndex = ComboBox.AddString(_T("Skeleton (White)"));

	if (ListIndex >= 0) {
		ComboBox.SetItemData(ListIndex, MWESM_NPCFLAG_BLOODSKEL);
	}

	ListIndex = ComboBox.AddString(_T("Metal (Gold)"));

	if (ListIndex >= 0) {
		ComboBox.SetItemData(ListIndex, MWESM_NPCFLAG_BLOODMETAL);
	}
}
...
void FillEsmRankCombo(CComboBox &ComboBox, const bool IncludeEmpty) {
	...
	if (ResultIndex >= 0) {
		ComboBox.SetItemData(ResultIndex, 0);
	}
...
bool SelectEsmIcon(CString &IconString, const TCHAR *pTitle, CWnd *pParent) {
	...
	if (Result >= 0) {
		Buffer.Delete(Result + 1, Buffer.GetLength() - Result - 1);
	}
	...
	if (Result == 0) {
		IconString.Delete(0, IconPath.GetLength());
	}
...
bool SelectEsmModel(CString &ModelString, const TCHAR *pTitle, CWnd *pParent) {
	...
	if (Result == 0) {
		ModelString.Delete(0, ModelPath.GetLength());
	}
...
bool SelectEsmSound(CString &SoundString, const TCHAR *pTitle, CWnd *pParent) {
	...
	if (Result == 0) {
		SoundString.Delete(0, SoundPath.GetLength());
	}
...
bool SelectEsmTexture(CString &TextureString, const TCHAR *pTitle, CWnd *pParent) {
	...
	if (Result == 0) {
		TextureString.Delete(0, TexturePath.GetLength());
	}
