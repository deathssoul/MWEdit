...
void CEsmApparatusDlg::GetControlData() {
	...
	if (Index >= 0) {
		pAppaData->Type = m_TypeList.GetItemData(Index);
	}

	/* Item quality */
	m_QualityText.GetWindowText(Buffer);
	pAppaData->Quality = (float)std::atof(Buffer);

	/* Item weight */
	m_WeightText.GetWindowText(Buffer);
	pAppaData->Weight = (float)std::atof(Buffer);

	/* Item value */
	m_ValueText.GetWindowText(Buffer);
	pAppaData->Value = std::atoi(Buffer);
