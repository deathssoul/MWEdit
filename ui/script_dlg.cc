...
CEsmScriptDlg::~CEsmScriptDlg() {
	if (m_ScriptAccel != NULL) {
		DestroyAcceleratorTable(m_ScriptAccel);
	}
...
void CEsmScriptDlg::GetControlData() {
	...
	if (pSubRec == NULL) {
		pSubRec = m_pScript->AllocateSubRecord(MWESM_SUBREC_SCVR);
	}
	...
	if (pSubRec == NULL) {
		pSubRec = m_pScript->AllocateSubRecord(MWESM_SUBREC_SCDT);
	}
...
void CEsmScriptDlg::OnChangeScriptText() {
	...
	if (m_UpdatingRichEdit || GetEsmOptNoScriptFormat()) {
		return;
	}
...
void CEsmScriptDlg::OnContextMenu(CWnd *pWnd, CPoint Point) {
	...
	if (pWnd->GetDlgCtrlID() != IDC_SCRIPTTEXT) {
		return;
	}
...
LRESULT CEsmScriptDlg::OnFindReplace(WPARAM wParam, LPARAM lParam) {
	...
	if (pDialog->MatchCase()) {
		Flags |= FR_MATCHCASE;
	}

	if (pDialog->MatchWholeWord()) {
		Flags |= FR_WHOLEWORD;
	}
...
void CEsmScriptDlg::OnSize(UINT nType, int cx, int cy) {
	...
	if (Width < ESMSCR_VIEW_MINSIZE) {
		Width = ESMSCR_VIEW_MINSIZE;
	}

	if (Height < ESMSCR_VIEW_MINSIZE) {
		Height = ESMSCR_VIEW_MINSIZE;
	}
...
int CEsmScriptDlg::ParseFuncToolTip(const TCHAR *pLineBuffer, const int CharPos) {
	...
	do {
		...
		if (StartPos <= CharPos && ParsePos >= CharPos) {
			break;
		}
	...
	if (pCurFuncInfo != NULL) {
		...
		if (ArgCount >= pFuncData->GetNumParams()) {
			ArgCount = pFuncData->GetNumParams() - 1;
		}
...
STDMETHODIMP CEsmScriptDlg::XRichEditOleCallback::QueryAcceptData(LPDATAOBJECT lpdataobj,
	...
	if (fReally && reco == RECO_DROP) {
		...
		if (pThis->m_LastDragStartSel >= 0 && pThis->m_LastDragEndSel >= 0 &&
			...
			if (StartSel > pThis->m_LastDragEndSel) {
