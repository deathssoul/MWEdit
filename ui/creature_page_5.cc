...
void CEsmCreaturePage5::GetControlData() {
	...
	if (pAiData != NULL) {
		...
		if (m_ApparatusCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_APPARATUS;
		}

		if (m_WeaponCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_WEAPONS;
		}

		if (m_ArmorCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_ARMOR;
		}

		if (m_LightCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_LIGHTS;
		}

		if (m_BookCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_BOOKS;
		}

		if (m_ClothCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_CLOTHING;
		}

		if (m_PickCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_PICKS;
		}

		if (m_ProbeCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_PROBES;
		}

		if (m_MiscCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_MISC;
		}

		if (m_SpellCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_SPELLS;
		}

		if (m_MagicCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_MAGICITEMS;
		}

		if (m_RepairCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_REPAIR;
		}

		if (m_IngreCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_INGREDIANTS;
		}

		if (m_SpellmakeCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_SPELLMAKING;
		}

		if (m_EnchantCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_ENCHANTING;
		}

		if (m_RepairsItemsCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_REPAIRITEM;
		}

		if (m_TrainCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_TRAINING;
		}

		if (m_PotionCheck.GetCheck()) {
			pAiData->Flags |= MWESM_CLASSAUTO_POTIONS;
		}
	}
}
...
void CEsmCreaturePage5::SetControlData() {
	...
	while (pDoorData != NULL) {
		...
		switch (Count) {
			case 0:
				m_CellList1.SetWindowText(Buffer);
				break;

			case 1:
				m_CellList2.SetWindowText(Buffer);
				break;

			case 2:
				m_CellList3.SetWindowText(Buffer);
				break;

			case 3:
				m_CellList4.SetWindowText(Buffer);
				break;
		}
