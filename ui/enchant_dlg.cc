...
void CEsmEnchantDlg::GetControlData() {
	...
	if (ListIndex >= 0) {
		m_pEnchant->SetEnchantType(m_TypeList.GetItemData(ListIndex));
	}
...
void CEsmEnchantDlg::GetEffectData(const int EffectIndex) {
	...
	if (ListIndex >= 0) {
		pEnchantData->RangeType = m_RangeList[EffectIndex].GetItemData(ListIndex);
	}
