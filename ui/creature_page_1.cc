...
void CEsmCreaturePage1::GetControlData() {
	...
	if (Index >= 0) {
		Flag |= m_BloodList.GetItemData(Index);
	}
	...
	if (Index >= 0) {
		pCreaData->Type = m_TypeList.GetItemData(Index);
	}
	...
	if (m_BipedCheck.GetCheck()) {
		Flag |= MWESM_CREAFLAG_BIPED;
	}

	if (m_NoneCheck.GetCheck()) {
		Flag |= MWESM_CREAFLAG_NONE;
	}

	if (m_SwimsCheck.GetCheck()) {
		Flag |= MWESM_CREAFLAG_SWIMS;
	}

	if (m_FliesCheck.GetCheck()) {
		Flag |= MWESM_CREAFLAG_FLIES;
	}

	if (m_WalksCheck.GetCheck()) {
		Flag |= MWESM_CREAFLAG_WALKS;
	}

	/* NPC flags */

	if (m_RespawnCheck.GetCheck()) {
		Flag |= MWESM_CREAFLAG_RESPAWN;
	}

	if (m_EssentialCheck.GetCheck()) {
		Flag |= MWESM_CREAFLAG_ESSENTIAL;
	}

	if (m_WeapShieldCheck.GetCheck()) {
		Flag |= MWESM_CREAFLAG_WEAPSHIELD;
	}
...
void CEsmCreaturePage1::OnAnimationbutton() {
	...
	if (Result) {
		m_AnimButton.SetWindowText(Buffer);
	}
}
