...
void CEsmSpellDlg::GetControlData() {
	...
	if (ListIndex >= 0) {
		m_pSpell->SetSpellType(m_TypeList.GetItemData(ListIndex));
	}
...
void CEsmSpellDlg::GetEffectData(const int EffectIndex) {
	...
	if (ListIndex >= 0) {
		pSpellData->RangeType = m_RangeList[EffectIndex].GetItemData(ListIndex);
	}
	...
	if (IsESMSkillEffect(pSpellData->EffectID)) {
		...
		if (ListIndex >= 0) {
			pSpellData->SkillID = (byte)m_SkillList[EffectIndex].GetItemData(ListIndex);
		}
	} else if (IsESMAttributeEffect(pSpellData->EffectID)) {
		ListIndex = m_SkillList[EffectIndex].GetCurSel();

		if (ListIndex >= 0) {
			pSpellData->AttributeID = (byte)m_SkillList[EffectIndex].GetItemData(ListIndex);
		}
	}
}
...
void CEsmSpellDlg::OnSelChangeEffectList(const int ListIndex) {
	...
	if (pEffectData->IsSelfOnly() || m_DisableCost) {
		...
		if (PrevSel >= 0) {
			PrevSel = m_RangeList[ListIndex].GetItemData(PrevSel);
		}
...
void CEsmSpellDlg::UpdateSpellCost(const int EffectIndex) {
	...
	if (ListIndex >= 0) {
		EffectData.EffectID = (short)m_EffectList[EffectIndex].GetItemData(ListIndex);
	}
...
void CEsmSpellDlg::UpdateTotalSpellCost() {
	...
	if (Index >= 0) {
		EnchantType = m_TypeList.GetItemData(Index);
	}
