...
void CMWEditDoc::CompareScripts(CEsmScript *pScript1, CEsmScript *pScript2) {
	...
	/* Ensure valid input */
	if (pScript1 == NULL || pScript2 == NULL) {
		return;
	}
...
bool CMWEditDoc::CompileActiveScript(CEsmScript *pScript) {
	...
	if (pSubRec == NULL) {
		pSubRec = pScript->AllocateSubRecord(MWESM_SUBREC_SCVR);
	}
	...
	if (pSubRec == NULL) {
		pSubRec = pScript->AllocateSubRecord(MWESM_SUBREC_SCDT);
	}
...
CEsmRecord *CMWEditDoc::CreateCopyINFO(CMWEditDoc *pDocument,
	...
	while (pRecInfo != NULL) {
		...
		if (pInfo->GetNextName()[0] == NULL_CHAR) {
			break;
		}
...
esmrecinfo_t *CMWEditDoc::GetInfoStart(CEsmDialogue *pDialRecord) {
	...
	while (pRecord != NULL) {
		...
		if (Index >= 0) {
			...
			if (pInfoRecord != NULL && pInfoRecord->IsType(MWESM_REC_INFO)) {
				break;
			}
...
bool CMWEditDoc::IsExistingIDDup(const TCHAR *pID) {
	...
	for (pRecInfo = m_RecInfoSort.GetFirstRecordKey(pID, Position); pRecInfo != NULL;
	     pRecInfo = m_RecInfoSort.GetNextRecordKey(pID, Position)) {
		/* Ignore certain record types */
		if (pRecInfo->pRecord->IsType(MWESM_REC_SNDG)) {
			continue;
		}
...
BOOL CMWEditDoc::OnOpenDocument(LPCTSTR lpszPathName) {
	...
	if (Result) {
		Result = LoadAppPlugins();
	}

	if (Result) {
		Result = LoadAppActive();
	}
...
int CMWEditDoc::OnPreSaveRecord(esmreceditinfo_t *pRecEditInfo) {
	...
	if (pRecEditInfo->HasNewID || pRecEditInfo->IsNew) {
		...
		if (!pRecEditInfo->HasAdded) {
			...
			if (!pRecEditInfo->IsCopied) {
				m_ActiveFile.AddRecord(pRecEditInfo->pRecInfo->pRecord);
			}
