...
void CEsmInfoDlg::GetControlData() {
	...
	if (m_pInfo->GetDialParent() != NULL
	    && m_pInfo->GetDialParent()->GetDialogTypeID() == MWESM_DIALTYPE_VOICE) {
...
void CEsmInfoDlg::GetFuncData(const int Index, int &FuncIndex) {
	...
	if (NewType == MWESM_SCVRFUNC_NONE) {
		return;
	}
...
void CEsmInfoDlg::OnSelChangeTypeList(const int Index) {
	...
	if (m_FuncVarList[Index].GetCurSel() < 0) {
		m_FuncVarList[Index].SetCurSel(0);
	}
}
