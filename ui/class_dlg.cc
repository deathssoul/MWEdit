...
void CEsmClassDlg::GetControlData() {
	...
	if (m_PlayableCheck.GetCheck()) {
		pClassData->Flags |= MWESM_CLASSFLAG_PLAYABLE;
	}

	if (m_WeaponCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_WEAPONS;
	}

	if (m_ArmorCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_ARMOR;
	}

	if (m_ClothingCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_CLOTHING;
	}

	if (m_BookCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_BOOKS;
	}

	if (m_IngrediantCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_INGREDIANTS;
	}

	if (m_PickCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_PICKS;
	}

	if (m_ProbeCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_PROBES;
	}

	if (m_LightCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_LIGHTS;
	}

	if (m_ApparatusCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_APPARATUS;
	}

	if (m_RepairCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_REPAIR;
	}

	if (m_MiscCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_MISC;
	}

	if (m_SpellCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_SPELLS;
	}

	if (m_MagicItemCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_MAGICITEMS;
	}

	if (m_PotionCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_POTIONS;
	}

	if (m_TrainCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_TRAINING;
	}

	if (m_SpellmakingCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_SPELLMAKING;
	}

	if (m_EnchantCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_ENCHANTING;
	}

	if (m_RepairItemsCheck.GetCheck()) {
		pClassData->AutoCalcFlags |= MWESM_CLASSAUTO_REPAIRITEM;
	}
}
	...
	for (Index = 0; Boxes[Index] != NULL; Index++) {
		...
		if (SkillID == CurrentSkillID) {
			for ( ; NextEmptySkill < MWESM_MAX_SKILLS; NextEmptySkill++) {
				if (SkillIDArray[NextEmptySkill] == 0) {
					break;
				}
			}
