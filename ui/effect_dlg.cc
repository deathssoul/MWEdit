...
void CEsmEffectDlg::GetControlData() {
	...
	if (Index >= 0) {
		m_pEffect->SetSchool(m_SchoolList.GetItemData(Index));
	}
...
bool CEsmEffectDlg::IsModified() {
	...
	if (m_SizeText.GetModify()) {
		m_Modified = true;
	}

	if (m_MaxSizeText.GetModify()) {
		m_Modified = true;
	}
	...
	if (m_RedText.GetModify()) {
		m_Modified = true;
	}

	if (m_GreenText.GetModify()) {
		m_Modified = true;
	}

	if (m_BlueText.GetModify()) {
		m_Modified = true;
	}
